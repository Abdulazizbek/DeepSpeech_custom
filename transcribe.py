...
from training.util.audio import AudioFile
from training.util.config import Config, initialize_globals
from training.util.feeding import split_audio_file
from training.util.flags import create_flags, FLAGS
from training.util.logging import log_error, log_info, log_progress, create_progressbar
...
def transcribe_file(audio_path, tlog_path):
    from training.train import create_model  # pylint: disable=cyclic-import,import-outside-toplevel
    from training.util.checkpoints import load_graph_for_evaluation
    initialize_globals()
    ...
    if not FLAGS.src or not os.path.exists(FLAGS.src):
        ...
        if os.path.isfile(src_path):
            if src_path.endswith('.catalog'):
                ...
                catalog_entries = [(resolve(catalog_dir, e['audio']), resolve(catalog_dir, e['tlog'])) for e in
                                   catalog_entries]
                ...
                if os.path.isfile(dst_path):
                    if FLAGS.force:
                        ...
                        fail('Destination file "{}" already existing - use --force for overwriting'.format(dst_path),
                             code=0)
